...
def load_enhanced_info_if_existed(num_heroes):
    ...
    return cooc_df.as_matrix(), against_wr_df.as_matrix(), global_wr_df.as_matrix().flatten(),\
           global_pop_df.as_matrix().flatten()

def archive_enhanced_info(cooccurrence, against_win_rate, global_win_rate, global_popularity):
    data_path = os.path.dirname(os.path.abspath(__file__)) + r'\data'
...
def build_enhanced_info(t1_pick, t2_pick, winners, num_heroes):
    ...
    archive_enhanced_info(cooccurrence, against_win_rate, global_win_rate, global_popularity)
